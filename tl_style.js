...
import { characters, getRequestHeaders, openCharacterChat, saveSettingsDebounced, getThumbnailUrl, getCurrentChatId } from "../../../../script.js";
...
/**
 * Highlights the path from the current chat position to root with a gold color.
 * Walks the graph structure from current node → root, choosing the correct parent
 * at branch points. Does NOT rely on file_name matching, which fails for shared nodes.
 *
 * @param {Object} cy - The Cytoscape instance.
 */
export function highlightCurrentPath(cy) {
    const context = getContext();
    const currentChatId = getCurrentChatId();
    if (!currentChatId || !context.chat || context.chat.length === 0) return;

    const currentMsgIndex = context.chat.length - 1;
    const currentFileName = currentChatId.endsWith('.jsonl') ? currentChatId : currentChatId + '.jsonl';

    const lastTarget = window.lastTimelineTarget;
    const upperBound = (lastTarget && lastTarget.chatId === currentChatId && lastTarget.messageId < currentMsgIndex)
        ? lastTarget.messageId
        : currentMsgIndex;

    // === Step 1: find the current-position node ===
    let currentNode = null;
    let maxIndex = -1;
    cy.nodes().forEach(node => {
        const sessions = node.data('chat_sessions') || [];
        const mi = node.data('messageIndex');
        if (sessions.includes(currentFileName)) {
            if (mi !== undefined && mi <= upperBound && mi > maxIndex) {
                currentNode = node;
                maxIndex = mi;
            }
        }
    ...
    // === Step 2: walk from current node up to root, collecting path nodes and edges ===
    const pathNodeIds = new Set();
    const pathEdges = [];
    let walker = currentNode;
    pathNodeIds.add(walker.id());

    while (true) {
        const parents = walker.incomers().nodes();
        if (parents.length === 0) break;

        let parent = parents[0];
        for (const p of parents) {
            const pSessions = p.data('chat_sessions') || [];
            if (pSessions.includes(currentFileName)) {
                parent = p;
                break;
            }
        }

        const edges = walker.incomers().edges();
        for (const e of edges) {
            if (e.data('source') === parent.id()) {
                pathEdges.push(e);
                break;
            }
        }
    ...
    // === Step 3: apply gold styles to path edges ===
    ...
    // === Step 4: apply subtle gold glow to path nodes ===
    cy.nodes().forEach(node => {
