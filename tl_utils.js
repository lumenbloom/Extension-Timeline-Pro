...
export async function navigateToMessage(chatSessionName, messageId, swipeId = -1, branch = false) {
    ...
    // Record the target for timeline gold-path highlight
    window.lastTimelineTarget = {
        chatId: chatSessionName,
        messageId: messageId,
    };
    ...
    try {
        ...
        if (!message.length) {
            // Message not in DOM at all — reveal any hidden .mes elements first
            chat.children('.mes').not(':visible').show();
            message = $(`div[mesid=${messageId}]`);
        }

        // If still not found, load more chat history via show_more_messages (async)
        let safety = 0;
        const MAX_LOADS = 50;
        while (!message.length && safety < MAX_LOADS) {
            const showMoreBtn = $('#show_more_messages');
            if (!showMoreBtn.length) break;
            console.info('Timelines: Loading more messages.');
            showMoreBtn.trigger('mouseup');
            // Yield to event loop so ST's async show_more handler can update the DOM
            await new Promise(r => setTimeout(r, 150));
            message = $(`div[mesid=${messageId}]`);
            safety++;
        }

        // Make message visible if it was hidden by lazy-load
        ...
        if (branch) {  // Create and open a new branch at target message (and open its target swipe, if any)
            ...
            if (message.length) {  // found the div?
                ...
                // Use native scrollIntoView for reliability, wait for layout
                const msgEl = message.get(0);
                if (msgEl) {
                    setTimeout(() => {
                        msgEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    }, 100);
                } else {
                    console.warn('Timelines: message DOM element not found for scroll');
                }
            } else {
        ...
        // closeOpenDrawers removed: it globally toggled ST drawer states, breaking the top menu
    }
}
...
export function closeModal() {
    ...
    if (window.activeCytoscapeInstance) {
        console.log("Closing modal: Destroying Cytoscape instance.");
        
        // Destroy all tippy instances
        if (window.activeTippies) {
            window.activeTippies.forEach(tip => {
                if (tip && typeof tip.destroy === 'function') {
                    try {
                        tip.destroy();
                    } catch (e) {
                        console.warn("Error destroying tippy on close:", e);
                    }
                }
            });
        ...
        try {
            window.activeCytoscapeInstance.destroy();
        } catch (e) {
            console.warn("Error destroying Cytoscape instance:", e);
        }
        
        window.activeCytoscapeInstance = null;
        console.log("Cytoscape instance destroyed on modal close.");
    }

    // Reset the timeline state completely
    window.lastContext = null;
    window.lastTimelineData = null;

    // Clear the diagram div content
...
export function closeTippy() {
    ...
    // Also hide any tracked tippy instances
    if (window.activeTippies) {
        window.activeTippies.forEach(tip => {
            if (tip && typeof tip.hide === 'function') {
                try {
                    ...
                    console.warn("Error hiding tippy:", e);
                }
            }
        });
    }
...
export function handleModalDisplay() {
    ...
    //   - An outer div (`myModal`, the "outer modal") covers the whole viewport (hence, it catches the click here)
