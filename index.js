...
window.activeCytoscapeInstance = null;
window.activeTippies = new Set();
window.lastContext = null;
window.lastTimelineData = null;
...
const extensionName = "Extension-Timeline-Pro";
...
loadFile(`${extensionFolderPath}cytoscape.min.js`, 'js');
...
import { setupStylesAndData, highlightElements, restoreElements, highlightCurrentPath } from './tl_style.js';
...
function makeTippy(ele, text) {
	...
	// Track active tippies
	window.activeTippies.add(tip);
...
function makeTapTippy(ele) {
	...
	var tip = tippy(dummyDomEle, {
		...
		content: function () {
			...
			// --- Header: name row with role indicator ---
			...
			badge.classList.add('tl-role-badge');
			var isUser = ele.data('is_user');
			badge.textContent = isUser ? '👤' : '🤖';
			badge.title = isUser ? 'User' : 'Character';
			...
			nameEl.innerHTML = ele.data('name');
			headerRow.appendChild(nameEl);
			div.appendChild(headerRow);

			// --- Sub-line: date + optional swipes ---
			...
			dateEl.innerHTML = ele.data('send_date');
			metaRow.appendChild(dateEl);

			if (ele.data('totalSwipes') > 0) {
				var swipeBadge = document.createElement('span');
				swipeBadge.classList.add('tl-swipe-badge');
				swipeBadge.textContent = `↻ ${ele.data('totalSwipes')}`;
				swipeBadge.style.cssText = 'font-size:11px;background:rgba(128,128,128,0.15);padding:1px 8px;border-radius:10px;color:var(--SmartThemeBodyColor,#999);opacity:0.7';
				metaRow.appendChild(swipeBadge);
			}
			...
			menuLabel.style.cssText = 'font-size:12px;color:var(--SmartThemeBodyColor,#999);opacity:0.6;margin-bottom:6px';
			menuLabel.textContent = 'Sessions';
			...
			if (ele.data('chat_sessions')) {
				ele.data('chat_sessions').forEach((session, index) => {
					...
					let displayName = session.split('.jsonl')[0];
					let shortName = displayName.length > 30 ? displayName.substring(0, 28) + '…' : displayName;
					btn.innerHTML = `<span style="flex:1">${shortName}</span><span style="font-size:11px;opacity:0.4;margin-left:8px">📄</span>`;
					// Store full name in data attribute
					btn.dataset.sessionIndex = index;
					...
					branchBtn.classList.add('branch_button');
					branchBtn.innerHTML = '↗';
					branchBtn.title = 'Branch from ' + session.replace('.jsonl', '');
					branchBtn.dataset.sessionIndex = index;
		...
		onShow(instance) {
			...
			window.activeTippies.add(instance);
		},
		onHide(instance) {
			isTapTippyActive = false;
			window.activeTippies.delete(instance);
...
function createLegend(cy) {
	...
	header.style.cssText = 'font-size:12px;font-weight:600;margin-bottom:6px;color:var(--SmartThemeBodyColor,#ddd);opacity:0.7';
...
function setupEventHandlers(cy, nodeData) {
	...
	cy.on('render', function () {
		...
		// Apply gold path on the first render AFTER orientation is set
		if (!hasHighlightedPath && hasSetOrientation) {
			hasHighlightedPath = true;
			// Small delay to let layout/rendering settle
			setTimeout(() => {
				if (cy && !cy.destroyed()) {
					console.log('Timelines [highlight]: applying gold path');
					highlightCurrentPath(cy);
				}
			}, 200);
		}
	});
...
function renderCytoscapeDiagram(nodeData) {
	...
	if (window.activeCytoscapeInstance) {
		console.log("Destroying existing Cytoscape instance.");
		// Destroy context menus if the plugin provides a destroy method (good practice)
		// While cytoscape-context-menus doesn't have a standard destroy, destroying cy usually handles it.
		// However, explicitly destroying tippy instances managed by us is good.
		window.activeCytoscapeInstance.destroy(); // This is the key cleanup step
		window.activeCytoscapeInstance = null; // Clear the reference
		// Also clear any custom tippy instances if they persist
		window.activeTippies.forEach(tip => {
			if (tip && typeof tip.destroy === 'function') {
				tip.destroy();
			}
		});
		window.activeTippies.clear();
	}
	...
	if (cy) {
		...
		window.activeCytoscapeInstance = cy; // Store the new instance
		console.log("New Cytoscape instance stored.");
	} else {
...
async function updateTimelineDataIfNeeded() {
	...
	if (!window.lastContext || window.lastContext.characterId !== context.characterId) {
		let data = {};
		...
		if (!context.characterId) {
			...
			if (groupID) {
				...
				window.lastTimelineData = await prepareData(data, true);
			}
		}
		else {
			data = await fetchData(context.characters[context.characterId].avatar);
			window.lastTimelineData = await prepareData(data);
		}

		window.lastContext = context; // Update the lastContext to the current context
...
async function onTimelineButtonClick() {
	...
	handleModalDisplay();
	if (dataUpdated || !window.lastTimelineData) {
		renderCytoscapeDiagram(window.lastTimelineData);
	}
...
function onInputChange(element, settingName, rgbaValue = null) {
	...
	window.lastContext = null; // Invalidate the last context to force a data update
	saveSettingsDebounced();
}